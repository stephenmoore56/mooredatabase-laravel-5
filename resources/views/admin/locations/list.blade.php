	...
	<div class="col-md-8">
		...
		<table id="locationList" class="table table-striped table-condensed table-bordered">
			...
			@foreach ($locations as $location)
				<tr>
					<td>{{ $location->location_name }}, {{ $location->county_name }}
						County, {{ $location->state_code }} {{ $location->country_code }}</td>
					...
					<td class="text-center">
						@if($location->trips == 0)
							{{ Form::open(array(
								'route' => array('locations.destroy', $location->id),
								'method' => 'DELETE',
								'id' => 'form_deleteLocation'.$location->id)) }}
							<button type="submit" class="btn btn-danger btn-xs deleteLocation"
									id="deleteLocation{{ $location->id }}">
